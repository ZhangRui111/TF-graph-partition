...
def bottleneck(inputs, depth, depth_bottleneck, stride, rate=1,
    ...
    :param outputs_collections: Collection to add the ResNet unit output.
    ...
    with tf.variable_scope(scope, 'bottleneck_v2', [inputs]) as sc:
        ...
        residual = slim.conv2d(
            residual, depth, [1, 1], stride=1, normalizer_fn=None, activation_fn=None, scope='conv3')

        output = shortcut + residual

        return slim.utils.collect_named_outputs(
            outputs_collections, sc.name, output)
...
def resnet_v2(inputs,
    ...
    :param blocks: A list of length equal to the number of ResNet blocks. Each element
      is a resnet_utils.Block object describing the units in the block.
    :param num_classes: Number of predicted classes for classification tasks.
      If 0 or None, we return the features before the logit layer.
    :param is_training: whether batch_norm layers are in training mode.
    ...
    :param spatial_squeeze: if True, logits is of shape [B, C], if false logits is
